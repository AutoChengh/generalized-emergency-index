...
def build_argparser():
    ...
    parser.add_argument(
        ...
        nargs=2,
        metavar=("START", "END"),
        default=VISUALIZE_TIME_RANGE,
        help=(
            "Time range to visualize in seconds after the first frame. "
            f"Default: {VISUALIZE_TIME_RANGE[0]} {VISUALIZE_TIME_RANGE[1]}."
        ),
    )
    parser.add_argument(
        "--frame-step",
        type=int,
        default=FRAME_STEP,
        help=f"Render every Nth frame. Default: {FRAME_STEP}.",
    )
    parser.add_argument(
        "--gei-max",
        type=float,
        default=GEI_MAX,
        help=f"Upper colorbar bound for GEI. Default: {GEI_MAX}.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(OUTPUT_GIF_DIR),
        help=f"Directory for generated GIFs. Default: {OUTPUT_GIF_DIR}.",
    )
    parser.add_argument(
        "--map-offset",
        type=float,
        ...
        metavar=("DX", "DY"),
        default=MAP_OFFSET,
        help=f"SIND map offset in meters. Default: {MAP_OFFSET[0]} {MAP_OFFSET[1]}.",
    )
...
def main():
    ...
    global FRAME_STEP, VISUALIZE_TIME_RANGE, GEI_MAX, GEI_NORM, OUTPUT_GIF_DIR, MAP_OFFSET

    if args.frame_step < 1:
        parser.error("--frame-step must be at least 1.")
    if args.time_range[1] < args.time_range[0]:
        parser.error("--time-range END must be greater than or equal to START.")
    if args.gei_max <= GEI_MIN:
        parser.error("--gei-max must be greater than 0.")

    FRAME_STEP = args.frame_step
    VISUALIZE_TIME_RANGE = [float(args.time_range[0]), float(args.time_range[1])]
    GEI_MAX = float(args.gei_max)
    GEI_NORM = Normalize(vmin=GEI_MIN, vmax=GEI_MAX)
    OUTPUT_GIF_DIR = Path(args.output_dir)
    MAP_OFFSET = (float(args.map_offset[0]), float(args.map_offset[1]))
    ...
    print(f"Visualization time range: {VISUALIZE_TIME_RANGE}")
    print(f"Frame step: {FRAME_STEP}")
    print(f"GEI colorbar max: {GEI_MAX}")
    print(f"GIF output directory: {OUTPUT_GIF_DIR}")
